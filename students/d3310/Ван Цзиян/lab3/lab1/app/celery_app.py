"""
Конфигурация приложения Celery
Для асинхронной обработки задач
"""
from celery import Celery
from app.core.config import settings

# Создание экземпляра приложения Celery
celery_app = Celery(
    "lab3_parser",
    broker=settings.REDIS_URL,
    backend=settings.REDIS_URL,
    include=["app.tasks.parser_tasks"]
)

# Конфигурация Celery
celery_app.conf.update(
    task_serializer="json",
    accept_content=["json"],
    result_serializer="json",
    timezone="UTC",
    enable_utc=True,
    task_track_started=True,
    task_time_limit=30 * 60,  # 30 минут максимальное время выполнения задачи
    task_soft_time_limit=25 * 60,  # 25 минут мягкий лимит времени
    worker_prefetch_multiplier=1,
    worker_max_tasks_per_child=1000,
    result_backend_transport_options={
        'master_name': 'mymaster',
    },
)

# Конфигурация маршрутизации задач
celery_app.conf.task_routes = {
    "app.tasks.parser_tasks.*": {"queue": "parser_queue"},
}

# Конфигурация очередей
celery_app.conf.task_default_queue = "default"
celery_app.conf.task_queues = {
    "default": {
        "exchange": "default",
        "routing_key": "default",
    },
    "parser_queue": {
        "exchange": "parser",
        "routing_key": "parser",
    },
}

if __name__ == "__main__":
    celery_app.start()
